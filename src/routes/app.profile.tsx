...
import {
  BadgeCheck, Grid3x3, Bookmark, Heart, Settings, Share2, MapPin, LogOut, Play, Loader2,
} from "lucide-react";
import { AppShell } from "@/components/app/AppShell";
import { useAuth } from "@/lib/auth";
import { useShellRole } from "@/hooks/use-shell-role";
import {
  useProfileStats, useUserListings, useUserBookmarks, useUserLikedListings,
    ...
    <AppShell role={role} compact>
      <div className="flex flex-col items-center gap-4 sm:flex-row sm:items-end sm:gap-6">
        <div className="relative shrink-0">
          {profile?.avatar_url ? (
            <img src={profile.avatar_url} alt="" className="h-20 w-20 sm:h-24 sm:w-24 rounded-full object-cover ring-4 ring-background shadow" />
          ) : (
            <span className="grid h-20 w-20 sm:h-24 sm:w-24 place-items-center rounded-full bg-card font-serif text-3xl text-primary ring-4 ring-background shadow">
              {name[0].toUpperCase()}
            </span>
          )}
          {profile?.verified && (
            <span className="absolute bottom-0 right-0 grid h-6 w-6 place-items-center rounded-full bg-primary text-primary-foreground ring-2 ring-background">
        ...
        <div className="min-w-0 flex-1 text-center sm:text-left">
          <h1 className="font-serif text-2xl sm:text-3xl text-foreground">{name}</h1>
      ...
      <div className="mt-5 flex items-center justify-center gap-8 sm:justify-start sm:gap-10 text-center">
