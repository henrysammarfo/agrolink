...
function Cart() {
  const [items, setItems] = useState(
    listings.slice(0, 3).map((l) => ({ ...l, qty: 5 }))
  );
          ...
          {items.map((it) => (
            <div key={it.id} className="flex items-center gap-4 rounded-2xl border border-border bg-card p-4">
              ...
              <div className="flex items-center gap-1 rounded-full border border-border p-1">
                <button onClick={() => setItems((arr) => arr.map((x) => x.id === it.id ? { ...x, qty: Math.max(1, x.qty - 1) } : x))} className="grid h-7 w-7 place-items-center rounded-full hover:bg-background">
                  <Minus className="h-3.5 w-3.5" />
                </button>
                <span className="w-8 text-center text-sm">{it.qty}</span>
                <button onClick={() => setItems((arr) => arr.map((x) => x.id === it.id ? { ...x, qty: x.qty + 1 } : x))} className="grid h-7 w-7 place-items-center rounded-full hover:bg-background">
                  <Plus className="h-3.5 w-3.5" />
                </button>
              </div>
              <button onClick={() => setItems((arr) => arr.filter((x) => x.id !== it.id))} className="grid h-9 w-9 place-items-center rounded-full text-muted-foreground hover:text-destructive">
        ...
        <aside className="rounded-3xl border border-border bg-card p-6">
          ...
          <div className="mt-6">
            ...
            <div className="mt-3 grid grid-cols-3 gap-2 text-xs">
              {["MTN MoMo", "Vodafone", "AirtelTigo"].map((p, i) => (
                <button key={p} className={`rounded-xl border py-3 ${i === 0 ? "border-primary bg-primary/10" : "border-border"}`}>{p}</button>
              ))}
            </div>
          </div>

          <button className="mt-8 inline-flex w-full items-center justify-center gap-2 rounded-full bg-foreground py-3.5 text-sm font-medium text-background hover:bg-foreground/90">
            Pay GHS {total} <ArrowRight className="h-4 w-4" />
          </button>
          <p className="mt-4 text-center text-xs text-muted-foreground">Secured by Hubtel · You'll receive a USSD prompt.</p>
        </aside>
      </div>
    </AppShell>
  );
