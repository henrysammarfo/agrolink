              ...
              <div key={o.id} className="py-3">
                <div className="flex items-center justify-between">
                  <div className="min-w-0">
                    <div className="truncate text-sm font-medium">{o.id.slice(0, 8)}</div>
                    <div className="truncate text-xs text-muted-foreground">
                      {o.items?.map((i) => `${i.listing?.title ?? "Item"} ×${i.quantity}`).join(", ")}
                    </div>
                  </div>
                  <div className="text-right">
                    <div className="text-sm text-primary">GHS {o.total_amount}</div>
        ...
        <div className="mt-5 grid gap-4 sm:grid-cols-2 lg:grid-cols-4">
          {listings.slice(0, 4).map((l) => (
            <div key={l.id} className="overflow-hidden rounded-2xl border border-border bg-background">
              ...
              <div className="p-4">
                <div className="truncate font-serif text-lg">{l.title}</div>
                <div className="mt-1 flex items-center justify-between text-xs">
                  <span className="text-muted-foreground">{l.quantity}{l.unit}</span>
                  <span className="text-primary">GHS {l.price_per_unit}/{l.unit}</span>
                </div>
              </div>
            </div>
          ))}
          {listings.length === 0 && (
            <div className="col-span-full py-8 text-center text-sm text-muted-foreground">Create your first listing.</div>
          )}
        </div>
      </section>
    </SellerStudioLayout>
