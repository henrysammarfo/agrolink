...
type SearchResult = {
  ...
  farmers: { id: string; display_name: string; slug: string | null; username?: string | null; region: string | null; is_driver?: boolean; is_seller?: boolean; vehicle_type?: string | null }[];
            ...
            {results.farmers.filter((f) => f.slug || f.username || f.id).map((f) => (
              <CommandItem key={f.id} onSelect={() => go("/app/users/$slug", { slug: f.username ?? f.slug ?? f.id })}>
                ...
                <span className="ml-auto text-xs text-muted-foreground">
                  {f.is_seller ? "Seller" : f.is_driver ? "Driver" : "User"} · @{f.username ?? f.slug?.replace(/-/g, "")}
                </span>
              </CommandItem>
            ))}
          </CommandGroup>
