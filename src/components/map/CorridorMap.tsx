...
export function CorridorMap({
  pins, route, className = "", center = [5.65, 0.05], zoom = 10, height = "100%",
  animateDriver, driverLabel = "Driver", onProgress, dark,
}: Props) {
  ...
  useEffect(() => {
    ...
    pins.forEach((p) => {
      ...
      L.marker([p.lat, p.lng], { icon }).bindTooltip(p.label, { direction: "top", offset: [0, -10] }).addTo(map);
    });

    if (route && route.length > 1) {
      L.polyline(route, { color: dark ? "#22c55e" : "#0b3d2e", weight: 4, opacity: 0.85, lineCap: "round" }).addTo(map);
      L.polyline(route, { color: "#ffffff", weight: 1.5, opacity: 0.5, dashArray: "2 8" }).addTo(map);
    }

    if (animateDriver && route && route.length > 1) {
      ...
      const marker = L.marker(route[0], { icon: driverIcon }).bindTooltip(driverLabel, { direction: "top", offset: [0, -14] }).addTo(map);
      driverRef.current = marker;

      // segment lengths for smooth movement
      const segs = route.slice(1).map((pt, i) => {
        const [a, b] = [route[i], pt];
        return Math.hypot(b[0] - a[0], b[1] - a[1]);
      });
      const total = segs.reduce((s, x) => s + x, 0);
      const duration = 22000; // ~22s loop
...
// Short farm→buyer corridor used for a single order tracking view
